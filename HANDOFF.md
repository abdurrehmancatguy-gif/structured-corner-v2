...
| **Test site (Netlify)** | https://corner-database-test.netlify.app/ - the only Netlify site that deploys |
| **First Netlify site** | https://enchanting-daffodil-aca9b0.netlify.app/ - frozen on an old build, skipped by netlify.toml |
...
### Netlify
Netlify builds nothing: the owner asked for none, and the pages are committed, so a deploy
is the repository as it stands. It publishes the repository ROOT, where `_redirects`
refuses the generator, the content, the tools and the stylesheet sources with a 404 and
rewrites everything else into `flow/` (forced, `200!`). So nothing checks there that the
committed pages match a fresh build: run `build.py` before pushing. `/assets/*` is cached
for a year as immutable, HTML and `sw.js` as no-cache, so every asset URL carries
`?v=<content hash>` (`V()` and `PV()` in `build.py`; product photos share their original's
version, passed to `shop.js` in `catalogue.js`).

Only **corner-database-test** deploys: `ignore = "test \"$SITE_NAME\" != corner-database-test"`,
and Netlify cancels a build whose ignore command exits 0, so the first Netlify site stays
frozen on what it already has. `netlify.toml` holds the old publish directory and build
command in a comment, to put back when Netlify should build again.

A new address also has to be added in Auth0 (application **bgs-corner**): the callbacks
`<url>/account.html` and `<url>/account`, the logout `<url>/` and the web origin `<url>`.
`admin/tests/test_signin.py` keeps the same three lists and walks each address, so the test
says what Auth0 must hold.

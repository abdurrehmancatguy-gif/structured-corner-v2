...
└── flow/                   ←←← THE SITE. Its HTML, favicon.ico, robots.txt and assets/ are published.
    ...
    ├── assets/
    │   ├── flow.css        1937 lines - append-only; has duplicate-selector debt
    │   ├── shop.js         1681 lines - vanilla JS, 24 independent bgsRun blocks
...
`admin/README.md` lists the 13 files with their port variables. Each runs on a temporary
...
Vanilla JS, no build step, 24 self-contained `bgsRun` blocks. Each is independent; one
...
`bgsFill` for page text, `catText` for category text, and `BGS_AR` and `BGS_QUIZ`. Each
...
20. **Arabic follows the exact English.** The Arabic dictionary is keyed by the exact English
   text, so editing a label drops its Arabic.

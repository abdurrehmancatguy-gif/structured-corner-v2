...
Only build.py, make_derivatives.py and make_favicon.py, with a fixed argv, a
clean environment and timeouts; never the importers, which read folders
outside the repo and rewrite products.json wholesale. After a run the flow/
tree is compared with how it was before, so a tool that touched anything
outside its own outputs fails the save instead of slipping through.
"""
import os
import subprocess
import time
...
TOOLS = {
    ...
    "favicon": (["tools/make_favicon.py"], 60),
}
